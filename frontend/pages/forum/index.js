...
function ForumPage(props) {
    ...
    async function getPosts() {
        console.log(tags);
        await fetch(`${process.env.NEXT_PUBLIC_API_URL}/meta/forum/posts?` + new URLSearchParams({
            tags: tags,
            sortBy: sortBy,
            query: query,
            direction: direction,
        ...
        <div className={styles["forum"]}>
            ...
            <div className={styles["forum-header-container"]}>
                <form className={styles["search-bar"]} onSubmit={handleSubmit}>
                    ...
                    <input placeholder="Search by title, content, or tags" value={query} onChange={(e) => setQuery(e.target.value)}></input>
                </form>
                <Link href="/forum/create"><FontAwesomeIcon className={styles["create"]} icon={faPlus} size="lg" /></Link>
            </div>
                <select className={styles["filters"]} name="sortBy" id="sortBy" value={sortBy} onChange={(e) => setSortBy(e.target.value)}>
                    <option value="Date">Date</option>
                    <option value="Votes">Votes</option>
                </select> 
                <select className={styles["filters"]} name="direction" id="direction" value={direction} onChange={(e) => setDirection(e.target.value)}>
                    <option value="Asc">Asc</option>
                    <option value="Desc">Desc</option>
                </select> 
            <div className={styles["posts-container"]}>
                {posts.map(post => {
                    return <PostPreview title={post.title} content={post.content} date={post.date} replyCount={post.comment_count} votes={post.votes} tags={post.tags}/>
                })}
            </div>
        </div>
    )
