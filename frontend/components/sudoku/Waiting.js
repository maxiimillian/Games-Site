...
function Waiting(props) {
  ...
  useEffect(() => {
    setUrl(`https://${window.location.hostname}/sudoku/${props.code}`);
  }, []);
  ...
  function generateOptionsList(options) {
    return options.map((option) => {
      return (
        <li key={option} className={styles["waiting-option"]}>
          {option}
        </li>
      );
    });
    ...
    <div className={`box ${styles["waiting-container"]}`}>
      <div className={styles["waiting-subcontainer"]}>
        <div className={styles["waiting-header"]}>
          <h1>{props.code}</h1>
          <FontAwesomeIcon
            className={sidebarStyles["left-option-icon"]}
            icon={faUserFriends}
            fixedWidth
          />
          <span className={styles["waiting-player-count"]}>
            {props.player_count}/{props.player_total}
          </span>
        </div>
        ...
        <div className={styles["input-container"]}>
          ...
          <FontAwesomeIcon
            ...
            className="clipboard hover-pointer"
            size="2x"
            icon={wasClicked ? faClipboardCheck : faClipboard}
            fixedWidth
          />
        </div>
