...
module.exports = function(io) {
    class ServerInformation {
        ...
        get_room_code_by_user_id(user_id) {
            for (let [room_code, game] of Object.entries(this.room_tracker)) {
        ...
        add_player_to_game(room_code, socket) {
            ...
            if (room_code == null) {
                ...
                socket.emit("err", "Room doesn't exit");
            }
            else if (this.get_room_code_by_user_id(user_id) == room_code && room_code ) { // for rejoining after leaving the game, second condition to protect against user_id not existing and null room code	
                socket.join(room_code);
