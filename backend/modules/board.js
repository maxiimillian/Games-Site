...
const DEFAULT_BOARD = {
  id: null,
  unsolved:
    "385926471472815936169374800058437129723691548914258763231749685897563214546182397",
  solved:
    "385926471472815936169374852658437129723691548914258763231749685897563214546182397",
  difficulty: "easy",
};
...
module.exports = class Board {
  ...
  static create(difficulty) {
    return new Promise((resolve, reject) => {
      utils
        ...
        .then((board) => {
          const unsolved = board.unsolved;
          const solved = board.solved;
          const base_clues = getIndex(board.unsolved);
          resolve(new Board(unsolved, solved, base_clues));
        })
        .catch((err) => {
          reject(err);
        });
  ...
  set_board(board) {
    if (board) {
      if (board.id && board.solved && board.unsolved && board.difficulty) {
        this.board = board;
      } else {
